...
from app.models import Agent, Ticket, WorkflowRule
...
def seed_demo_data(db: Session) -> None:
    ...
    agents = [
        Agent(name="Ridhan P", team="Workforce Desk", skill="SLA monitoring", active_tickets=2, avg_resolution_hours=5.5, productivity_score=94),
        Agent(name="Anna K", team="Billing Ops", skill="Billing investigations", active_tickets=4, avg_resolution_hours=7.2, productivity_score=88),
        Agent(name="Markus L", team="Technical Support", skill="API and platform issues", active_tickets=3, avg_resolution_hours=6.1, productivity_score=91),
        Agent(name="Sofia R", team="Policy Ops", skill="Policy reviews", active_tickets=5, avg_resolution_hours=9.4, productivity_score=86),
        Agent(name="Emils B", team="Escalations Desk", skill="Critical escalations", active_tickets=1, avg_resolution_hours=4.8, productivity_score=96),
    ]
    db.add_all(agents)
    db.commit()
    ...
    workflows = [
        WorkflowRule(
            ...
            actions=[
                ...
                {"type": "notify", "audience": "operations_lead", "message": "Critical ticket requires immediate attention."},
            ],
        ),
        WorkflowRule(
            ...
            actions=[
                ...
                {"type": "notify", "audience": "workforce_desk", "message": "Ticket is at SLA risk. Review ownership and next action."},
            ],
        ),
        WorkflowRule(
